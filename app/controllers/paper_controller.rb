class PaperController < ApplicationController
  ...
  def generate
    pages_to_include = [
      ...
      ["Universe",    1], # single Universe page
    ]
  ...
  def individual
    ...
    raise "Invalid page type: #{page_type}" unless Rails.application.config.content_types[:all].include?(page_type)
  ...
  def paper_params
    params.require(:paper)
          .permit(*Rails.application.config.content_types[:all_non_universe])
          .reject { |klass_name, boolean| boolean == "0" }
  end
end
